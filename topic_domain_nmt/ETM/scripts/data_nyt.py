...
data_file = '/mnt/nas6/users/wangweixuan/data/wmt14-ende/train.de'
...
f = open('/mnt/nas6/users/wangweixuan/data/wmt14-ende/bin/dict.de.txt','r')
line = f.readlines()
vocab = []
i = 0
while i <len(line):
    ...
    i += 1
f.close()
print('  vocabulary size after removing stopwords from list: {}'.format(len(vocab)))
# Filter out stopwords (if any)
vocab = [w for w in vocab if w not in stops]

print('  vocabulary after removing stopwords: {}'.format(len(vocab)))
...
path_save = './min_df_' + str(min_df) + 'wmt_de' +'/'
